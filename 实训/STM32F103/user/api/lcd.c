...
void LCD_Init(void)
{
	...
	LCD_init_RGB();
	LCD_DisplayOn();        /* 开背光 */
	LCD_Display_Dir(1);		 	//默认为0:竖屏 1:横屏
	LCD_Clear(GRAYBLUE);	
}
...
函数功能：部分清屏
****************************************************/
void LCD_PartClear(u16 x1,u16 y1,u16 x2,u16 y2,u16 c)
{	 
	for(int i = x1; i < x2; i++)
	{
		for(int j = y1; j < y2; j++)
...
***** 返回值：	无
****************************************************************************/
void Draw_Text_8_32_Str(u32 x,u32 y,u16 backColor,u16 color,const unsigned char *chs)
{
	u32 i=0;
	while (*chs != '\0')
	{
		if (*chs > 0xa0)				//汉字内码都是大于0xa0
		{
			...
			{	
				...
				if ((*chs == hz_index[i*2]) && (*(chs+1) == hz_index[i*2+1]))
				{
					show_xXx(x,y,backColor,color,&hz[120*(i)],30,32);
					x += 30;
					break;
				}
			}
...
void Paint_Bmp(int x0,int y0,int width,int high,const unsigned char bmp[])
{
    ...
    for( y = y0 ; y < (high+y0) ; y++ )
    {
    	...
    	{
    		c = bmp[p+1] | (bmp[p]<<8) ;
			if ( ( (x) < LCD_WIDTH) && ( (y) < LCD_HEIGHT) )//竖屏显示
